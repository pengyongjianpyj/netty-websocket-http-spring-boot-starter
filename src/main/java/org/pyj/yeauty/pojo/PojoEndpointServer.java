...
import org.pyj.yeauty.support.AntPathMatcherWrapper;
import org.pyj.yeauty.support.DefaultPathMatcher;
import org.pyj.yeauty.support.MethodArgumentResolver;
import org.pyj.yeauty.support.PathVariableMethodArgumentResolver;
...
public class PojoEndpointServer {
  ...
  private static final Map<String, PojoMethodMapping> pathMethodMappingMap = new HashMap<>();
  private static final Set<WsPathMatcher> pathMatchers = new HashSet<>();
  private final ServerEndpointConfig config;

  public PojoEndpointServer(ServerEndpointConfig config) {
    this.config = config;
  }

  public boolean hasBeforeHandshake(Channel channel, String path) {
    org.pyj.yeauty.pojo.PojoMethodMapping methodMapping = getPojoMethodMapping(path, channel);
  ...
  public void doBeforeHandshake(Channel channel, FullHttpRequest req, String path) {
    org.pyj.yeauty.pojo.PojoMethodMapping methodMapping = null;
    ...
    org.pyj.yeauty.pojo.Session session = new org.pyj.yeauty.pojo.Session(channel);
  ...
  public void doOnOpen(Channel channel, FullHttpRequest req, String path) {
    ...
    if (implement == null) {
      ...
      org.pyj.yeauty.pojo.Session session = new Session(channel);
      channel.attr(SESSION_KEY).set(session);
    }
  ...
  public void addPathPojoMethodMapping(String path, PojoMethodMapping pojoMethodMapping) {
    ...
    for (MethodArgumentResolver onOpenArgResolver : pojoMethodMapping.getOnOpenArgResolvers()) {
      if (onOpenArgResolver instanceof PathVariableMethodArgumentResolver ||
          onOpenArgResolver instanceof org.pyj.yeauty.support.PathVariableMapMethodArgumentResolver) {
